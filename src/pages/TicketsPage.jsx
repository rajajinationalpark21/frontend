...
import { Ticket, DollarSign, Camera, Car, ShieldCheck, AlertCircle, HelpCircle, CheckCircle2, Loader2 } from 'lucide-react';
...
export default function TicketsPage({ onOpenBooking }) {
  ...
  const entranceFees = data.entranceFees || [];
  const gypsyRates = data.gypsyRates || [];
  const guideFees = data.guideFees || [];
    ...
    <div className="min-h-screen bg-white dark:bg-gray-950 transition-colors">
      ...
      <section className="relative py-20 bg-zinc-950 text-white overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src={safariImages.safariJeepTrail} alt="Safari vehicle entering Rajaji" className="w-full h-full object-cover" />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-white mb-6">Tickets & Entry Tariffs</h1>
            <p className="text-lg text-gray-300 leading-relaxed">Transparent, government-standard entry fees, Gypsy vehicle tariffs, and naturalist guide charges for Indian citizens and international guests.</p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
            <div className="flex items-start gap-3">
              <div className="w-9 h-9 rounded-lg bg-safari-500/20 text-safari-600 dark:text-safari-400 flex items-center justify-center shrink-0"><Ticket className="w-5 h-5" /></div>
              <div><span className="font-bold text-gray-900 dark:text-white block">Season Calendar</span><span className="text-gray-600 dark:text-gray-300 text-xs">{timings.openDates}</span></div>
            </div>
            <div className="flex items-start gap-3">
              <div className="w-9 h-9 rounded-lg bg-safari-500/20 text-safari-600 dark:text-safari-400 flex items-center justify-center shrink-0"><Car className="w-5 h-5" /></div>
              <div><span className="font-bold text-gray-900 dark:text-white block">Summer Shift Timings</span><span className="text-gray-600 dark:text-gray-300 text-xs">{timings.summer}</span></div>
            </div>
            <div className="flex items-start gap-3">
              <div className="w-9 h-9 rounded-lg bg-safari-500/20 text-safari-600 dark:text-safari-400 flex items-center justify-center shrink-0"><ShieldCheck className="w-5 h-5" /></div>
              <div><span className="font-bold text-gray-900 dark:text-white block">Winter Shift Timings</span><span className="text-gray-600 dark:text-gray-300 text-xs">{timings.winter}</span></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div>
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Charged per 3-hour shift at all official park entry gates (Chila, Motichur, Ranipur, Jhilmil).</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-gray-200 dark:border-gray-800 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-xl bg-safari-500/10 text-safari-600 dark:text-safari-400 flex items-center justify-center"><Car className="w-5 h-5" /></div>
              <div><h3 className="text-xl font-bold text-gray-900 dark:text-white">4x4 Gypsy Safari Hire</h3><span className="text-xs text-gray-500 dark:text-gray-400">Fixed rate per vehicle per shift</span></div>
            </div>
            <div className="space-y-3">
              {gypsyRates.map((gypsy, gIdx) => (
                <div key={gIdx} className="p-4 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700/80 flex items-center justify-between">
                  <div><span className="font-bold text-gray-900 dark:text-white text-sm block">{gypsy.zone}</span><span className="text-xs text-gray-500 dark:text-gray-400">{gypsy.capacity}</span></div>
                  <div className="text-right"><span className="text-lg font-extrabold text-safari-600 dark:text-safari-400">{gypsy.rate}</span></div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-gray-200 dark:border-gray-800 shadow-sm flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-3 mb-6">
                ...
                <div><h3 className="text-xl font-bold text-gray-900 dark:text-white">Naturalist Guide Charges</h3><span className="text-xs text-gray-500 dark:text-gray-400">Mandatory certified park guide per vehicle</span></div>
              </div>
              <div className="space-y-4">
                {guideFees.map((guide, gdIdx) => (
                  <div key={gdIdx} className="p-4 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700/80">
                    <div className="flex justify-between items-start mb-1"><span className="font-bold text-gray-900 dark:text-white text-sm">{guide.type}</span><span className="font-bold text-emerald-600 dark:text-emerald-400">{guide.fee}</span></div>
        ...
        <div className="bg-gray-50 dark:bg-gray-900 p-8 rounded-2xl border border-gray-200 dark:border-gray-800">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2"><AlertCircle className="w-5 h-5 text-safari-600" />Important Booking & Entry Notes</h3>
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-gray-200 dark:border-gray-800">
            <span className="text-xs text-gray-500 dark:text-gray-400 text-center sm:text-left">Questions regarding permit allocations or corporate/educational discounts?</span>
            <button onClick={onOpenBooking} className="px-6 py-3 rounded-xl bg-safari-500 hover:bg-safari-600 text-white font-bold text-xs uppercase tracking-wider transition shadow-sm w-full sm:w-auto">Book Safari Permit Now</button>
          </div>
        </div>
      </section>
    </div>
